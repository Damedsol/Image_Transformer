...
const IMAGE_PROCESSING_TIMEOUT =
	parseInt(process.env.IMAGE_PROCESSING_TIMEOUT || "30") * 1000;
...
export const processImage = async (
	...
	try {
		logger.debug(
			...
			"Executing processImageWithLimits with timeout",
		);
		// Race between processing and timeout
		const result = await Promise.race([
			processImageWithLimits(imageFile, options),
			timeoutPromise,
		]);
		...
		if (
			...
			throw new AppError(
				"Resource error processing image. The image might be too large.",
				413,
				{
...
const processImageWithLimits = async (
	...
	logger.debug(
		{ quality, format: options.format },
		"Applying format and quality",
	);
...
export const createZipFromImages = async (
	...
	logger.info(
		{ numFiles: processedImages.length, zipFileName },
		"Starting createZipFromImages",
	);
...
export const cleanTempFiles = (
	...
	logger.debug(
		{ files: filePaths, delayMs },
		"Scheduling temporary file cleanup",
	);
	setTimeout(() => {
		filePaths.forEach((filePath) => {
			...
			if (ensurePathIsWithinBoundary(filePath, tempDir)) {
				try {
					...
					if (fs.existsSync(filePath)) {
						fs.unlink(filePath, (unlinkErr) => {
							if (unlinkErr) {
								...
								logger.info(
									{ file: filePath },
									"Scheduled temporary file removed successfully",
								);
							}
						});
					} else {
					...
					logger.error(
						{ err, file: filePath },
						"Unexpected error during scheduled file cleanup",
					);
				}
			} else {
				logger.warn(
