...
const storage = multer.diskStorage({
	destination: function (
		...
		logger.debug(
			...
			"Determinando destino de subida",
		);
		cb(null, uploadsDir);
	},
...
const validateFileType = (
	...
	if (
		...
		logger.warn(
			{ filename: file.originalname, mimetype, extension },
			"Tipo de archivo rechazado",
		);
		cb(
			new AppError(
...
export const safelyDeleteFile = (filePath: string): boolean => {
	logger.debug(
		{ file: filePath },
		"Intentando eliminar archivo de forma segura",
	);
	try {
		...
		if (fs.existsSync(filePath)) {
			...
			logger.info(
				{ file: filePath },
				"Archivo eliminado exitosamente (safelyDeleteFile)",
			);
			return true;
		}
		logger.warn(
			{ file: filePath },
			"Archivo no encontrado para eliminar (safelyDeleteFile)",
		);
		return false;
	} catch (error) {
