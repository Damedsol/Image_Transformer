...
- **Production Nginx Proxy Routing and Security Hardening:**
  ...
  - **Docker Port Exposure Reduction (Zero-Exposure V3):** Removed public port mappings for both `backend` (port `3001`) and `frontend` (port `8080`) in `docker-compose.prod.yml`, replacing them with internal network directives (`expose: - "3001"` and `expose: - "80"`). This restricts container access exclusively within the shared Docker network, satisfying the Zero-Exposure VPS security standard where all public traffic is strictly managed by the global Nginx Proxy Manager container.
