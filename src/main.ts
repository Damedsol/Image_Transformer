...
function setupAccessibilityFeatures() {
	...
	function updateBodyClasses() {
		...
		if (prefersReducedMotion()) {
			...
			const reduceMotionCheckbox = document.getElementById(
				"reduce-motion",
			) as HTMLInputElement;
...
function setupAccessibilityDialog() {
	...
	const a11yButton = document.getElementById(
		"open-a11y-dialog",
	...
	if (!dialog || !a11yButton || !closeButton) {
		logApiError(
			...
			new Error("Accessibility elements not found"),
		);
		return;
	}
	...
	const highContrastToggle = dialog.querySelector(
		"#high-contrast",
	) as HTMLInputElement;
	...
	const dyslexicFontToggle = dialog.querySelector(
		"#dyslexic-font",
	) as HTMLInputElement;
	...
	const reducedMotionToggle = dialog.querySelector(
		"#reduced-motion",
	) as HTMLInputElement;
	...
	const textSpacingOptions = dialog.querySelectorAll(
		'input[name="text-spacing"]',
	);
	textSpacingOptions.forEach((option) => {
		...
		radio.addEventListener("change", () => {
			if (radio.checked) {
				...
				announcePreferenceChange(
					"Text spacing",
					getTextSpacingName(radio.value),
				);
			}
		});
	});
		...
		if ((event.ctrlKey || event.metaKey) && event.altKey) {
			switch (event.key.toLowerCase()) {
				...
				case "c":
					...
					event.preventDefault();
					highContrastCheckbox = document.getElementById(
						"high-contrast",
					) as HTMLInputElement;
				...
				case "d":
					...
					if (dropZone) {
						dropZone.click();
						announceToScreenReader(
							"File selection activated to upload an image",
						);
					}
					break;
...
function loadUserPreferences() {
	try {
		...
		if (dyslexicFont === "true") {
			...
			const dyslexicFontToggle = document.querySelector(
				"#dyslexic-font",
			) as HTMLInputElement;
		...
		if (reducedMotion === "true") {
			document.body.classList.add("reduced-motion");
			const reducedMotionToggle = document.querySelector(
				"#reduced-motion",
			) as HTMLInputElement;
